...
Single-file, paper-grade demo: VQE + parametrised Max-Cut family + ID vs FD on the *value function* F(lambda)
---------------------------------------------------------------------------------------------------------
...
- Inner objective:      J(phi,lambda) = <H(lambda)>_phi,   H(lambda)=Sum_e w_e(lambda) C_e,  C_e=(I-Z_i Z_j)/2.
- Value function:       F(lambda)   = max_phi J(phi,lambda)     (bilevel target; approximated by the inner solve).
- Classical diagnostic: J_cl^*(lambda) = max_z J(z;lambda)  (bitstring envelope; used only as a diagnostic scale).

FAIRNESS / CORE LOGIC (Claim 1)
- Evaluating F(lambda) is expensive: it requires an (approx.) inner solve at lambda.
- ID (implicit diff / envelope principle; correlator reuse):
    after ONE inner solve at lambda_t, compute  g_ID(lambda_t)=d_lambda J(phi*(lambda_t),lambda_t)=Sum_e w'_e(lambda_t) p_e,
    reusing the same ZZ correlators that were already measured for J.
- FD baseline is black-box on the value function:
    g_FD(lambda_t) ~ [F(lambda_t+c) - F(lambda_t-c)]/(2c),
    where each F(.) query requires an additional inner solve at lambda+-c.
  => FD has ~2 additional inner solves per outer step vs ID (~3x evaluation cost in this setup).
...
Run (example):
  python exp01_id_vs_fd_core_demo.py --kind periodic --n 12 --outer 100 --inner 30 --fmt pdf
"""
...
# ==============================================================================
# 1) Experiment-specific plotting helpers
# ==============================================================================
...
def plot_envelope_and_expectations(path: Path, lam_grid, J_cl_star, hist_id, hist_fd):
    """
    Fig1:
      - main axis: full classical envelope J_cl^*(lambda) + all trajectory points
    """
    ...
    ax.plot(lam_grid, J_cl_star, color=COLORS["ENV"], lw=2.0, label=r"Envelope $J_{\mathrm{cl}}^*(\lambda)$", zorder=1)

    # Scatter full trajectories (no connecting lines in (lambda, value))
    ax.scatter(
        hist_id["lam_pre"],
        hist_id["J"],
        s=10,
...
def plot_bestJ_vs_evals(path: Path, hist_id, hist_fd, J_cl_max=None, budget: float = None):
    ...
    ax.plot(ev_fd, y_fd, color=COLORS["FD"], lw=1.7, ls="--", label=r"VQE + FD (black-box $F$)")
...
def plot_evals_vs_outer(path: Path, hist_id, hist_fd, budget: float):
    ...
    ax.plot(t_fd, hist_fd["evals_cum"], color=COLORS["FD"], lw=1.6, ls="--", label=r"VQE + FD (black-box $F$)")
...
def plot_xray(path: Path, lams, all_J, J_cl_star, active_ids, switch_lams, switch_vals):
    ...
    ax.plot(lams, all_J[~is_active].T, color="#aaaaaa", alpha=0.05, lw=0.30, rasterized=True, label="_nolegend_")
    ax.plot(lams, all_J[is_active].T, color=COLORS["FD"], alpha=0.55, lw=1.0, rasterized=True, label="_nolegend_")

    ax.plot(
        lams, J_cl_star, color=COLORS["ENV"], lw=2.0, ls="--", label=r"Envelope $J_{\mathrm{cl}}^*(\lambda)$", zorder=5
    )
    if switch_lams.size:
        ax.scatter(
            switch_lams,
...
# ==============================================================================
# 2) Experiment-specific VQE helpers (different interface from shared version)
# ==============================================================================
...
# ==============================================================================
# 3) Classical envelope/spectrum + outer loops (ID vs FD on F)
# ==============================================================================
...
def run_outer(
    ...
    for t in range(1, outer + 1):
        ...
        # INNER SOLVE at current lambda  -> one approximate value-evaluation of F(lambda)
        w = fam.w(lam)
        ...
        # objective at lambda_t  (~ F(lambda_t))
        ...
        if mode == "ID":
            ...
            # FD on VALUE FUNCTION F(lambda) -> extra inner solves at lambda+-c
...
def parse_args():
    ...
    p.add_argument("--out", type=str, default="outputs/exp01_id_vs_fd_core_demo")
...
def main():
    ...
    fam = Family1D(len(edges), a.kind, (a.lam_min, a.lam_max), rng, periodic_K=a.periodic_K, safety_bounds=False)
    ...
    hist_fd = run_outer(
        ...
        fam,
        cut_mask,
        a.aam0 if hasattr(a, "aam0") else a.lam0,
        a.outer,
        a.inner,
        a.L,
        a.seed,
    ...
    plot_envelope_and_expectations(out / f"envelope_expectations_{suf}.{a.fmt}", lams, J_cl_star, hist_id, hist_fd)
    ...
    plot_lambda_trajectories(
        out / f"lambda_trajectories_{suf}.{a.fmt}",
        hist_id,
        hist_fd,
        lam_true=lam_true,
    ...
    if a.readout_shots > 0:
        ...
        ax.set_xlabel(r"$\lambda$")
        ax.set_ylabel("Best sampled cut")
        ax.legend(loc="lower right", frameon=False)
        _savefig(fig, out / f"best_cut_{suf}.{a.fmt}")
        plt.close(fig)
        ...
        ax.set_xlabel(r"$\lambda$")
        ax.set_ylabel("Mode cut")
        ax.legend(loc="lower right", frameon=False)
        _savefig(fig, out / f"mode_cut_{suf}.{a.fmt}")
        plt.close(fig)

    print("Saved to:", out.resolve())
    print(f"classical grid argmax lambda*~{lam_true:.4f}, max J_cl^*~{J_cl_max:.6f}")
