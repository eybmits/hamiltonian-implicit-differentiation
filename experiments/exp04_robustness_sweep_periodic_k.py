...
Example
  python exp04_robustness_sweep_periodic_k.py \
...
# ==============================================================================
# 1) Experiment-specific figure size
# ==============================================================================
...
# ==============================================================================
# 2) Helpers: AUC, stats, CSV/TEX (experiment-specific signatures)
# ==============================================================================
...
# ==============================================================================
# 3) Classical diagnostic (experiment-specific)
# ==============================================================================


def classical_switch_density_and_scale(
    fam: Family1D, cut_mask: np.ndarray, grid_points: int
...
# ==============================================================================
# 4) VQE expectation values (experiment-specific wrappers)
# ==============================================================================
...
def value_eval(
    n: int, edges, Z: np.ndarray, fam: Family1D, lam: float, params0: np.ndarray, L: int, inner_iters: int, seed: int
):
    """
...
def run_bilevel_outer(
    ...
    for t in range(1, int(outer_max) + 1):
        ...
        if mode == "ID":
            ...
            # +c
            Jp, _, _, cost_p = value_eval(n, edges, Z, fam, lp, params.copy(), L, inner, seed=seed + 1000 * t + 1)
            evals += cost_p
            best = max(best, Jp)
            evals_evt.append(evals)
            best_evt.append(best)
            ...
            # -c
            Jm, _, _, cost_m = value_eval(n, edges, Z, fam, lm, params.copy(), L, inner, seed=seed + 1000 * t + 2)
            evals += cost_m
            best = max(best, Jm)
            evals_evt.append(evals)
            best_evt.append(best)
